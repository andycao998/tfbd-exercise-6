...
GROUNDED_PROMPT = """ 
You answer questions about our company's internal policies.
Rules:
    ...
    4. End your answer with a "Sources:" line naming the documents used.
"""

# R - retrieve relevant chunks from KB
def retrieve(query: str, max_results: int = 4) -> list[dict]:
    """ returns up to 'max_results' number of chunks from KB that are relevant to the 'query' """
...
# G - prompt the model with the freshly retrieved passages
def answer_with_sources(query: str, max_results: int = 4) -> dict:
    """ 
    calls retrieve and agent steps before sending prompt to model
        - with a managed KB, we have to manually do each of the steps
    ...
    return {
        ...
        "chunks": chunks, # useful for debugging
